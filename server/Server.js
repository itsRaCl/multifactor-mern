...
// Function to send to the user's email
app.post("/login", async (req, res) => {
  ...
  if (user) {
    ...
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: "turing.x.nerd@gmail.com",
        pass: "nomx zaoq ybpo ntpl",
      },
    });

    const mailOptions = {
      from: "turing.x.nerd@gmail.com",
      to: email,
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      res.status(200).send("User Valid, OTP sent successfully.");
    });
  } else {
